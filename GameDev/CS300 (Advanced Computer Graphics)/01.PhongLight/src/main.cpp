...
void display(SDL_Window * window, GLsizei Width, GLsizei Height, std::vector<Object*> shape,
	GLSLProgram * ShaderProgram, GLuint texture, Camera & camera, Light * light, Material * material1, Material * material2)
{
	...
	ShaderProgram->SetUniform("TextureData", 0);
...
int main(int argc, char* args[])
{
	...
	while (!quit)
	{
		...
		if (!PauseLight)
		{
			...
			LightShape->SetPosition(glm::rotate(0.01f, glm::vec3(0.0f, 1.0f, 0.0f)) * glm::vec4(LightShape->GetPosition(), 1.0f));

			//Light movement on y
			LightShape->UpdatePosition(glm::vec3(0.0f, 0.4f * cosf(angle / 2.0f), 0.0f));
		}
		...
		//Render the shapes
		display(window, WIDTH, HEIGHT, Objects, ShaderProgram, texture, camera, Light1, Material1, Material2);
	}
