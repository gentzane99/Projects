...
void DrawTriangleSolid(const Point2D & p0, const Point2D & p1, const Point2D & p2, sf::Image & image, std::vector<float> & ZBuffer_)
{
	...
	//Z
	float v1Z[3] = { middle->val[0] - top->val[0], middle->val[1] - top->val[1], static_cast<float>(middle->val[2] - top->val[2]) };
	float v2Z[3] = { bottom->val[0] - top->val[0], bottom->val[1] - top->val[1], static_cast<float>(bottom->val[2] - top->val[2]) };
    ...
    while (y <= yMax)
    {
        ...
        while (x <= xMax)
        {
			//Checks if the position of x and y is on the screen
			if ((unsigned)x < image.getSize().x && x > 0 && (unsigned)y < image.getSize().y && y > 0 &&
			z < ZBuffer_[image.getSize().x * y + x])
			{
				ZBuffer_[image.getSize().x * y + x] = z;
