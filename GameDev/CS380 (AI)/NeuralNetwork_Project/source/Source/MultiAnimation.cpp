...
#define	IDC_SNAKEGAME			47
#define	IDC_SNAKEGAMESPEED		48
#define	IDC_SNAKEGAMESPEEDTEXT	49
...
void InitApp()
{
	...
	//g_SampleUI.AddButton(IDC_SNAKEGAME, L"Play Snake Game", 45, iY += 26, 120, 24);
...
//	g_OccupancyMapUI.AddSlider(IDC_UPDATEDISTANCE, 5, iY += 26, 120, 24, 1, 10, static_cast<int>(DEFAULT_FOVCLOSEDISTANCE));
	...
	g_OccupancyMapUI.AddStatic(IDC_SNAKEGAMESPEEDTEXT, L"Game speed", 5, iY -= 200, 120, 24);
	g_OccupancyMapUI.AddSlider(IDC_SNAKEGAMESPEED, 8, iY += 26, 120, 24, 0, 1000, static_cast<int>(DEFAULT_SPEED * 1000));
//#endif
...
void RenderText()
{
	...
	txtHelper.DrawFormattedTextLine(L"Times Died:                  %d", g_snakeGame.m_died_count);		    //Times that Snake died
	txtHelper.DrawFormattedTextLine(L"Max Apple Eaten:        %d"     , g_snakeGame.m_max_apple_count);	    //Max apple eaten
	txtHelper.DrawFormattedTextLine(L"Apple Eaten in Total:   %d"     , g_snakeGame.m_total_apple_count);	//Apple eaten in total
	txtHelper.DrawFormattedTextLine(L"Current game speed:     %f"     , g_snakeGame.m_time_to_move);		//Snake game speed each step
	txtHelper.DrawFormattedTextLine(L"Current epsilon:        %f"     , g_QLearning.m_epsilon_value);		//Posibility from 1 to 0 of computing a random action

	//txtHelper.DrawFormattedTextLine(L"up   :   %f"     , g_QLearning.m_network_main.m_neuron_layers[g_QLearning.m_network_main.m_number_of_layers-1][0].m_output);
	//txtHelper.DrawFormattedTextLine(L"right:   %f"     , g_QLearning.m_network_main.m_neuron_layers[g_QLearning.m_network_main.m_number_of_layers-1][1].m_output);
	//txtHelper.DrawFormattedTextLine(L"down :   %f"     , g_QLearning.m_network_main.m_neuron_layers[g_QLearning.m_network_main.m_number_of_layers-1][2].m_output);
	//txtHelper.DrawFormattedTextLine(L"left :   %f"     , g_QLearning.m_network_main.m_neuron_layers[g_QLearning.m_network_main.m_number_of_layers-1][3].m_output);
...
void CALLBACK OnGUIEventOccupancyMap(UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext)
{
	...
	case IDC_SNAKEGAMESPEED:

		g_snakeGame.m_time_to_move = ((float)reinterpret_cast<CDXUTSlider*>(pControl)->GetValue()) / 1000.0f;
